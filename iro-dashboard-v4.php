<?php
/**
 * Template Name: IRO Mission Control Live Chat (v4.0)
 */
...
<body class="h-screen flex flex-col overflow-hidden selection:bg-cyber-cyan/30">
    ...
    <script>
        ...
        function handleChatSubmit(e) {
            ...
            // Fetch real-time AI response from your local PM2 Backend
            ...
            .then(data => {
                if (data && data.reply) {
                    ...
                    simulateBotError('No response received from IRO brain.');
                }
            })
            .catch(e => {
                simulateBotError('Connection to localhost:3004/api/chat failed.');
            });
        ...
        function simulateBotReply(replyText) {
            ...
            botBlock.innerHTML = `
                <div class="flex-1 space-y-2">
                    <div class="text-[10px] font-bold text-cyber-cyan px-2 flex items-center gap-1">
                        <i data-lucide="cpu" class="w-3 h-3"></i> IRO / AI DIRECT
                    </div>
                    <div class="message-bubble bot p-4 shadow-lg text-sm text-white/90">
                        ${replyText.replace(/\n/g, '<br>')}
                    </div>
        ...
        function simulateBotError(errMsg = 'Cannot reach local node server') {
            const chatWindow = document.getElementById('chat-window');
            const anchor = document.getElementById('chat-anchor');
            const botBlock = document.createElement('div');
            botBlock.className = 'flex justify-start max-w-3xl mb-6 fade-in';
            botBlock.innerHTML = `
                <div class="flex-1 space-y-2">
                    <div class="text-[10px] font-bold text-[#FF5C00] px-2 flex items-center gap-1">
                        <i data-lucide="alert-triangle" class="w-3 h-3"></i> SYSTEM ERROR
                    </div>
